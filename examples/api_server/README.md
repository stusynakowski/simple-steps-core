...
| `POST /workflows` | Validate + create a workflow from `{step_id, operation_id, arguments}` steps. |
...
curl -s localhost:8000/workflows -X POST -H 'content-type: application/json' -d '{
  ...
  "workflow_id": "wf-001",
  "steps": [
    {"step_id": "step_nums",   "operation_id": "make_list", "arguments": {"n": 10}},
    {"step_id": "step_squared","operation_id": "map", "arguments": {"over": "step_nums", "op": "slow_square", "concurrency": 4, "on_error": "collect"}},
    {"step_id": "step_total",  "operation_id": "total", "arguments": {"data": "step_squared.ok"}}
  ]
}'
...
- **Author steps** as `{step_id, operation_id, arguments}`. To reference an
  earlier step, name it `step_*` and pass its id as a **quoted string** value in
  `arguments`, e.g. `"data": "step_nums"` or `"data": "step_squared.ok"`.
