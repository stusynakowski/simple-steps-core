...
def test_valid_references_have_no_issues():
    wf, registry = _wf()
    wf.add(Operation(step_id="step_nums", name="make_list", arguments={"n": 5}))
...
def test_unknown_step_is_flagged():
    wf, registry = _wf()
    wf.add(Operation(step_id="step_sum", name="total", arguments={"data": "step_missing"}))
...
def test_forward_reference_is_flagged():
    ...
    wf.add(Operation(step_id="step_sum", name="total", arguments={"data": "step_nums"}))
    wf.add(Operation(step_id="step_nums", name="make_list", arguments={"n": 3}))
    issues = check_reference_types(wf, registry)
...
def test_type_mismatch_is_flagged():
    ...
    wf.add(Operation(step_id="step_lbl", name="label", arguments={"text": "step_nums"}))
    issues = check_reference_types(wf, registry)
...
def test_map_over_reference_is_compatible():
    ...
    wf.add(Operation(step_id="step_sum", name="total", orchestration={"mode": "collapse", "over": "step_nums", "initial": 0}))
